...
(function () {
    ...
    function modal(title, body, buttons) {
        ...
        const actions = overlay.querySelector(".gv-modal-actions");
        buttons.forEach(({ label, className = "", onClick }) => {
            const b = document.createElement("button");
            b.type = "button"; b.className = `gv-modal-action ${className}`; b.textContent = label;
            b.addEventListener("click", () => onClick(overlay, b)); actions.appendChild(b);
        });
        const close = () => overlay.remove();
        overlay.querySelector(".gv-modal-x").addEventListener("click", close);
    ...
    menu.querySelector("#gv-delete-audios").addEventListener("click", () => {
        ...
        modal("DELETE ALL SAVED AUDIOS?", `<p>This permanently deletes <strong>all encrypted recordings</strong> saved to your GhostVoice account. Your account will remain active.</p><p class="gv-warning">This action cannot be undone.</p>`, [
            { label: "CANCEL", onClick: (overlay) => overlay.remove() },
            { label: "DELETE ALL", className: "gv-modal-danger", onClick: async (overlay, b) => {
                b.disabled = true; b.textContent = "DELETING…";
                try {
                    const result = await api("/api/profile/delete-audios", { method: "POST" });
                    overlay.querySelector(".gv-modal-body").innerHTML = `<p><strong>${result.deleted || 0}</strong> saved recording(s) deleted.</p>`;
                    b.textContent = "DONE";
                    setTimeout(() => overlay.remove(), 900);
                } catch (error) { b.disabled = false; b.textContent = "DELETE ALL"; overlay.querySelector(".gv-modal-body").innerHTML += `<p class="gv-error">${error.message}</p>`; }
            }}
        ]);
    });
    ...
    menu.querySelector("#gv-delete-account").addEventListener("click", () => {
        ...
        const overlay = modal("DELETE ACCOUNT", `<p>This permanently deletes your GhostVoice account, encrypted recordings, password-reset records and account audit data.</p><label class="gv-modal-label"><span>ACCOUNT PASSWORD</span><input id="gv-delete-password" type="password" minlength="12" autocomplete="current-password" placeholder="Enter your password"></label><p class="gv-error" id="gv-delete-error"></p>`, [
            { label: "CANCEL", onClick: (overlay) => overlay.remove() },
            { label: "DELETE ACCOUNT", className: "gv-modal-danger", onClick: async (overlay, b) => {
                const password = overlay.querySelector("#gv-delete-password").value;
                if (!password) { overlay.querySelector("#gv-delete-error").textContent = "Enter your password."; return; }
                b.disabled = true; b.textContent = "VERIFYING…";
                try {
                    await api("/api/profile/delete-account", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password }) });
                    overlay.remove(); window.location.href = "/";
                } catch (error) { b.disabled = false; b.textContent = "DELETE ACCOUNT"; overlay.querySelector("#gv-delete-error").textContent = error.message; }
            }}
        ]);
        setTimeout(() => overlay.querySelector("#gv-delete-password")?.focus(), 50);
    });

    menu.querySelector("#gv-logout").addEventListener("click", async () => {
        closeMenu();
    ...
    async function loadProfile() {
        try {
            ...
            nameElement.textContent = profile.username || "ACCOUNT";
            emailElement.textContent = profile.email || "No recovery email";
            adminLink.hidden = !profile.is_admin;
        } catch (_) {}
    }

    const observer = new MutationObserver(() => {
        if (!app.classList.contains("hidden")) loadProfile();
    });
